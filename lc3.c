...
// extend x from 'bit_count' to 16 bits (two's complement)
uint16_t sign_extend(uint16_t x, int bit_count)
{
    // if x has negative sign bit
    if ((x >> (bit_count - 1)) & 1)
    {
        x |= (0xFFFF << bit_count);
    }
    ...
    while (running)
    {
		...
		uint16_t r1 = (instr >> 6) & 0x7;
            ...
            case OP_LD: // load
            {
                ...
                // PCoffset 9
                uint16_t pc_offset = sign_extend(instr & 0x1FF, 9);
                // load memory at reg[R_PC] + pc_offset into R0
		...
		// load memory at reg[base_r] + offset into BaseR
		...
		uint16_t r0 = (instr >> 9) & 0x7;
                // PCoffset 9
                uint16_t pc_offset = sign_extend(instr & 0x1FF, 9);
                // write R0 to reg[R_PC] + pc_offset
		...
		uint16_t r0 = (instr >> 9) & 0x7;
                // PCoffset 9
                uint16_t pc_offset = sign_extend(instr & 0x1FF, 9);
                // find address stored at reg[R_PC] + pc_offset, write R0 to that address location
